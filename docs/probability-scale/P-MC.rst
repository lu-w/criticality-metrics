...
.. math::
		\mathit{P}\text{-}\mathit{MC} = P(\mathcal{C}) = \int P(\mathcal{C}~|~\mathcal{U}) P(\mathcal{U}) d\mathcal{U}

with :math:`P(\mathcal{C}~|~\mathcal{U})` denoting the probability of collision under a certain set of inputs.
